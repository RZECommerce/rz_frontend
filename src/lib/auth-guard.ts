...
export async function requireAuth() {
  ...
  const { user, fetchUser, loading, isLoading, isAuthenticated } = state;
  ...
  while ((currentState.loading || currentState.isLoading) && waitCount < maxWait) {
    await new Promise((resolve) => setTimeout(resolve, 50));
    currentState = useAuthStore.getState();
    waitCount++;
    
    // Check if we became authenticated while waiting
    if (currentState.isAuthenticated && currentState.user) {
      return;
    }
  }

  // Final check - if still not authenticated, redirect to login
  currentState = useAuthStore.getState();
  if (!currentState.isAuthenticated || !currentState.user) {
    throw redirect({
      to: "/login",
...
export async function requirePermission(permission: string) {
  ...
  // Wait for permissions to be available (check both JSON structure and flat array)
  while (
    Object.keys(rbacState.permissions).length === 0 && 
    rbacState.permissionsFlat.length === 0 && 
