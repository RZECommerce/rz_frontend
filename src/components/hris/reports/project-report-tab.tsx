    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select date range for project report</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle className="text-2xl">{report.summary?.total_projects || 0}</CardTitle>
              </CardHeader>
            </Card>
            <Card>
              <CardHeader className="pb-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Project records from {filters.start_date} to {filters.end_date}</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <Table>
                  ...
                  <TableBody>
                    {report.projects && report.projects.length === 0 ? (
                      ...
                      report.projects?.map((project: any, index: number) => (
                        <TableRow key={project.id || index}>
                          <TableCell className="font-medium">{project.project_name}</TableCell>
                          <TableCell>{project.start_date}</TableCell>
                          <TableCell>{project.end_date}</TableCell>
                          <TableCell>{project.team_members || 0}</TableCell>
                          <TableCell>{project.progress ? `${project.progress}%` : "0%"}</TableCell>
                          <TableCell>
