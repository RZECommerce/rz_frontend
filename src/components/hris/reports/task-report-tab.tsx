...
export function TaskReportTab() {
  ...
  const { data: report, isLoading } = useQuery({
    ...
    queryFn: () => reportService.getTaskReport(filters),
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select date range for task report</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle className="text-2xl">{report.summary?.total_tasks || 0}</CardTitle>
              </CardHeader>
            </Card>
            <Card>
              <CardHeader className="pb-2">
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle className="text-2xl">{report.summary?.in_progress || 0}</CardTitle>
              </CardHeader>
            </Card>
            <Card>
              <CardHeader className="pb-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Task records from {filters.start_date} to {filters.end_date}</CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    {report.tasks && report.tasks.length === 0 ? (
                      ...
                      report.tasks?.map((task: any, index: number) => (
                        <TableRow key={task.id || index}>
                          ...
                          <TableCell>{task.progress ? `${task.progress}%` : "0%"}</TableCell>
                          <TableCell>
