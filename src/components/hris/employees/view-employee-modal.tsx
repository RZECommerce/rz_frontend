...
import { commissionService } from "@/services/core-hr.service";
...
export function ViewEmployeeModal({
  ...
  const { data: salaryComponents, isLoading: isLoadingSalaryComponents } = useQuery({
    ...
    enabled: open && !!employeeId && (activeTab === "allowances" || activeTab === "other-payment"),
  });
  ...
  const { data: commissionsData, isLoading: isLoadingCommissions } = useQuery({
    ...
    enabled: open && !!employeeId && activeTab === "commissions",
  });
...
function PayslipDetails({ payslip, employee, formatCurrency, formatDate }: PayslipDetailsProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Allowances */}
      {(payslip.commission_taxable > 0 || payslip.allowance_taxable > 0 || payslip.allowance_non_taxable > 0) && (
        <Collapsible open={openSections.allowances} onOpenChange={() => toggleSection("allowances")}>
          <CollapsibleTrigger className="w-full">
            <div className="flex items-center justify-between p-3 border rounded-lg hover:bg-muted/50">
              <span className="font-semibold">Allowances & Commission</span>
          ...
          <CollapsibleContent>
            <div className="border-t rounded-b-lg p-4 bg-background">
              <Table>
                <TableBody>
                  {payslip.commission_taxable > 0 && (
                    <TableRow>
                      <TableCell className="font-medium">Commission (Taxable):</TableCell>
                      <TableCell className="text-right">{formatCurrency(payslip.commission_taxable)}</TableCell>
                    </TableRow>
                  )}
                  {payslip.allowance_taxable > 0 && (
                    <TableRow>
                      <TableCell className="font-medium">Allowance (Taxable):</TableCell>
                      <TableCell className="text-right">{formatCurrency(payslip.allowance_taxable)}</TableCell>
                    </TableRow>
                  )}
                  {payslip.allowance_non_taxable > 0 && (
                    <TableRow>
                      <TableCell className="font-medium">Allowance (Non-Taxable):</TableCell>
                      <TableCell className="text-right">{formatCurrency(payslip.allowance_non_taxable)}</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
