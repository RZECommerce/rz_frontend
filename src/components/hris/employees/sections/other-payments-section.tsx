...
const otherPaymentSchema = z.object({
  component_type: z.enum(["bonus", "other"]),
...
type OtherPaymentFormData = z.infer<typeof otherPaymentSchema>;
...
export function OtherPaymentsSection({ employeeId, isEditMode = true }: OtherPaymentsSectionProps) {
  ...
  const { data: salaryComponents, isLoading } = useQuery({
    queryKey: ["employee-salary-components", employeeId, "other"],
    queryFn: () => salaryComponentService.getByEmployee(employeeId),
  });
  ...
  const otherPayments = React.useMemo(() => {
    ...
    return components.filter((sc: SalaryComponent) =>
      !["allowance", "commission"].includes(sc.component_type || "")
    );
  }, [salaryComponents]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Other payment added successfully");
      setIsAddDialogOpen(false);
      reset();
    },
    onError: (error: Error) => {
      toast.error("Failed to add other payment", { description: error.message });
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["employee-salary-components", employeeId] });
      toast.success("Other payment updated successfully");
      setIsAddDialogOpen(false);
      setEditingId(null);
      reset();
    },
    onError: (error: Error) => {
      toast.error("Failed to update other payment", { description: error.message });
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["employee-salary-components", employeeId] });
      toast.success("Other payment deleted successfully");
    },
    onError: (error: Error) => {
      toast.error("Failed to delete other payment", { description: error.message });
    },
  });
  ...
  React.useEffect(() => {
    if (editingId) {
      const payment = otherPayments.find((p) => p.id === editingId);
      if (payment) {
        reset({
          component_type: (payment.component_type === "bonus" ? "bonus" : "other") as "bonus" | "other",
          name: payment.name,
          calculation_type: payment.calculation_type,
          amount: payment.amount || 0,
          percentage: payment.percentage || 0,
          is_taxable: payment.is_taxable,
          effective_date: payment.effective_date,
          end_date: payment.end_date || "",
          description: payment.description || "",
        });
        setCalculationType(payment.calculation_type);
        setIsAddDialogOpen(true);
      }
    }
  }, [editingId, otherPayments, reset]);
    ...
    <div className="space-y-4">
      ...
      {isLoading ? (
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          Loading other payments...
        </div>
      ) : otherPayments.length > 0 ? (
        <div className="space-y-4">
          {otherPayments.map((payment: SalaryComponent) => (
                  ...
                  {isEditMode && (
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => setEditingId(payment.id)}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className="font-medium">
                    {payment.calculation_type === "percentage"
                      ? `${payment.percentage}%`
                ...
                <div>
                  ...
                  <span className="font-medium">{payment.is_taxable ? "Yes" : "No"}</span>
                </div>
                <div>
                  <span className="text-muted-foreground">Effective Date: </span>
                  <span className="font-medium">{formatDate(payment.effective_date)}</span>
                </div>
                {payment.end_date && (
                  <div>
                    <span className="text-muted-foreground">End Date: </span>
                    <span className="font-medium">{formatDate(payment.end_date)}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <p className="text-sm text-muted-foreground">No other payments found.</p>
      )}

      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingId ? "Edit Other Payment" : "Add Other Payment"}</DialogTitle>
            <DialogDescription>
              {editingId ? "Update payment information" : "Add a new payment for this employee"}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setValue("component_type", value as "bonus" | "other")}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Input id="name" {...register("name")} placeholder="Enter payment name" />
